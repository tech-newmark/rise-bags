  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    createBasketItemContent: function (data) {
      ...
      if (
        ...
        titleHtml = `<a class="bx-soa-item-title" href="${data.DETAIL_PAGE_URL}">${itemImg}<span>${this.htmlspecialcharsEx(itemName)}</span></a>`;
      } else {
      ...
      if (this.options.showPropsInBasket && props.length) {
        props.forEach((prop) => {
          propsNodes += `<div class='bx-scu-prop bx-soa-item-prop-row'><span>${prop.NAME}</span><span>${prop.VALUE}</span></div>`;
        });
      }

...
?>
	...
	<div class="product-item-container" id="<?= $areaId ?>" data-entity="item">
		...
		$file = new Main\IO\File($documentRoot . $templateFolder . '/item-template.php');
		if ($file->isExists()) {
			// Подключение шаблона карточки товара
		...
		if (!$haveOffers) {
			...
			// Добавляю также в ТП все изображения из товара
			$itemPhotos = [];

			// if (is_array($item["PROPERTIES"]["MORE_PHOTO"]["VALUE"])) {
			// 	foreach ($item["PROPERTIES"]["MORE_PHOTO"]["VALUE"] as $arPhotoId) {
			// 		$itemPhotos[] = CFile::GetFileArray($arPhotoId);
			// 	}
			// }
			$jsParams = array(
				'PRODUCT_TYPE' => $item['PRODUCT']['TYPE'],
				...
				'PRODUCT' => array(
					...
					'NAME' => $productTitle,
					'DETAIL_PAGE_URL' => $item['DETAIL_PAGE_URL'],
					'MORE_PHOTO' => $item['MORE_PHOTO'], // array_merge($item['MORE_PHOTO'], $itemPhotos),
					'MORE_PHOTO_COUNT' => $item['MORE_PHOTO_COUNT']
				),
				'OFFERS' => array(),
				'OFFER_SELECTED' => 0,
